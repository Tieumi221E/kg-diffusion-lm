...
from scripts.datasets import PromptResponseDataset, SFTMaskCollator
from scripts.eval_utils import evaluate_loss
...
def main():
    ...
    train_loader = build_dataloader(Path(data_cfg["train_jsonl"]), tokenizer, collator_cfg, int(data_cfg["batch_size"]), True, max_length)
    eval_loader = build_dataloader(Path(data_cfg["eval_jsonl"]), tokenizer, collator_cfg, int(data_cfg["eval_batch_size"]), False, max_length)
    ...
    while step < int(train_cfg_raw["max_steps"]):
        ...
        if step % int(train_cfg_raw["save_every"]) == 0:
            ...
            cleanup_checkpoints(output_dir, int(config["logging"].get("keep_last_n_checkpoints", 1)))
