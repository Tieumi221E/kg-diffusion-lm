...
from scripts.train_utils import build_tokenizer, load_yaml, build_model, load_checkpoint
from scripts.eval_utils import evaluate_em_token
from scripts.eval_checkpoint_evalsplit import EVAL_SPLITS, build_eval_loader, resolve_eval_mask_settings
...
def main():
    ...
    for ckpt_path in checkpoints:
        ...
        for offset, (name, filename, inc_context) in enumerate(EVAL_SPLITS):
            ...
            loader = build_eval_loader(jsonl_path, inc_context, tokenizer, collator_cfg, max_length, 128, mask_mode, mask_option, 0, 42+offset, context_sep)
            em, _, _, _, _, _ = evaluate_em_token(model, loader, device, tokenizer, collator_cfg)
